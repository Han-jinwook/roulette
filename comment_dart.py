...
ENDED_TITLE_PREFIX = "(종료)"


def _title_has_ended_prefix(title: str) -> bool:
    return str(title or "").strip().startswith(ENDED_TITLE_PREFIX)


def _apply_ended_title_prefix(title: str) -> str:
    s = str(title or "").strip()
    if not s:
        return ENDED_TITLE_PREFIX
    if s.startswith(ENDED_TITLE_PREFIX):
        return s
    return f"{ENDED_TITLE_PREFIX} {s}"


def _remove_ended_title_prefix(title: str) -> str:
    s = str(title or "").strip()
    if s.startswith(ENDED_TITLE_PREFIX):
        return s.replace(f"{ENDED_TITLE_PREFIX} ", "", 1).replace(ENDED_TITLE_PREFIX, "", 1).strip()
    return s


def _sync_roulette_closed_state_from_db(event_id: str) -> str:
    """DB title 접두 기준으로 메모리 종료 상태를 복원한다. 반환: 종료 메시지 또는 ''."""
    if not event_id:
        return ""
    try:
        _, _, _, title, _, _, _, _, _, _ = db.get_data(event_id, include_commenters=False)
        if _title_has_ended_prefix(title):
            roulette_event_closed[event_id] = roulette_event_closed.get(event_id) or DEFAULT_ROULETTE_CLOSED_MESSAGE
            return roulette_event_closed[event_id]
    except Exception as e:
        print(f"DEBUG: [_sync_roulette_closed_state_from_db] {e}")
    roulette_event_closed.pop(event_id, None)
    return ""


def _persist_roulette_closed_title(event_id: str, closed: bool) -> bool:
    """종료 상태를 title 접두로 DB에 영속화한다."""
    if not event_id:
        return False
    try:
        participants, last_id, _, title, prizes, memo, winners, allow_duplicates, allowed_list, event_at = db.get_data(
            event_id, include_commenters=False
        )
        next_title = _apply_ended_title_prefix(title) if closed else _remove_ended_title_prefix(title)
        ok, err_detail = db.save_data_blocking(
            event_id,
            dict(participants or {}),
            last_id or "",
            title=next_title,
            prizes=prizes or "",
            memo=memo or "",
        ...
        if not ok:
            print(f"DEBUG: [_persist_roulette_closed_title] save failed: {err_detail}")
        return bool(ok)
    except Exception as e:
        print(f"DEBUG: [_persist_roulette_closed_title] {e}")
        return False


def _roulette_closed_message_for_event(event_id: str, title: str) -> str:
    """메모리 우선, 없으면 DB title 접두로 종료 상태 복원."""
    if not event_id:
        return ""
    msg = roulette_event_closed.get(event_id)
    if msg:
        return msg
    if _title_has_ended_prefix(title):
        roulette_event_closed[event_id] = DEFAULT_ROULETTE_CLOSED_MESSAGE
        return roulette_event_closed[event_id]
    return ""
...
def handle_end_roulette_event(data=None):
    ...
    _persist_roulette_closed_title(active_url, True)
    global games
    games.clear()
...
def handle_cancel_roulette_event(data=None):
    ...
    if active_url not in roulette_event_closed:
        restored = _sync_roulette_closed_state_from_db(active_url)
        if restored:
            roulette_event_closed[active_url] = restored
    if active_url not in roulette_event_closed:
        socketio.emit(
            'error',
        ...
        return
    _persist_roulette_closed_title(active_url, False)
...
def handle_start_rotation(data):
    ...
    active_url = get_active_url()
    if active_url and active_url not in roulette_event_closed:
        _sync_roulette_closed_state_from_db(active_url)
        ...
        try:
            ...
            if active_url:
                ...
                if not allow_duplicates:
                    ...
                    pass
                _EVENT_SYNC_CACHE.pop(active_url, None)
