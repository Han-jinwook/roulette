...
이전 로드맵([프로덕트-Plan.md](file:///d:/roulette/docs/%ED%94%84%EB%A1%9C%EB%8D%95%ED%8A%B8-Plan.md))에서 제시된 내용 중, 플랫폼 확장 및 상용화 품질 확보를 위해 향후 검토할 백로그 과제들입니다.

1. **상세 통계 대시보드 및 내보내기 (Phase 2):**
   - 모임별 일일 방문자(게스트) 통계, 최고 동시 접속자 수 트래킹.
   - 역대 당첨 기록 데이터의 **CSV 다운로드 기능** 제공.
2. **GUI 기반 테마 커스터마이징 (Phase 2):**
   - 운영자가 룰렛 원판의 색상 팔레트, 팡파레 효과음, 배경 스킨 이미지 등을 대시보드에서 직접 변경 및 실시간 반영할 수 있는 테마 빌더 기능.
3. **외부 플랫폼 연동 확장 Webhook (Phase 3):**
   - 네이버 카페 스크래핑 외에도 유튜브 슈퍼챗, 아프리카TV 별풍선, 치지직 치즈 등 방송 후원 이벤트 수신 시 자동으로 룰렛 명단에 티켓이 차감/충전되는 연동 API(Webhook) 지원.
4. **대규모 트래픽 분산 설계 (Phase 3):**
   - Redis Pub/Sub 기반 다중 Socket.IO 클러스터 구성을 도입하여 수만 명 이상의 대형 이벤트 동시 접속 수용.
5. **어뷰징 및 해킹 방지 (Phase 3):**
   - 비정상적 트래픽 유입에 대응하기 위한 IP 대역 차단 기능 및 매크로 방지 캡차(CAPTCHA) 검증 로직 도입.
